        ...
        registration.interceptors(new ChannelInterceptor() {
            ...
            public Message<?> preSend(Message<?> message, MessageChannel channel) {
                ...
                if (accessor != null && StompCommand.CONNECT.equals(accessor.getCommand())) {
                    String playerCode = accessor.getFirstNativeHeader("X-Player-Code");
                    if (playerCode != null) {
                        String playerId = gameService.getPlayerId(playerCode);
                        String gameCode = gameService.getGameCodeForPlayer(playerCode);
                        if (playerId != null) {
                            accessor.getSessionAttributes().put("playerId", playerId);
                            accessor.getSessionAttributes().put("playerCode", playerCode);
                            if (gameCode != null) {
